...
<head>
    ...
    <title>Laporan Data Ruangan Kelas</title>
    ...
    <div class="container-fluid">
        ...
        <div class="row mb-4">
            <div class="col-12 text-center">
                <h5 class="fw-bold text-decoration-underline">LAPORAN DATA RUANGAN KELAS AKTIF</h5>
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach($classrooms as $idx => $c)
                            <tr>
                                ...
                                <td class="fw-bold text-center">{{ $c->kelas->nama }} - {{ $c->rombel->nama }}</td>
                                <td>{{ $c->guru->nama_lengkap ?? '-' }}</td>
                                <td class="text-center">{{ $c->semester->nama }}</td>
                                <td class="text-center">{{ $c->tahun_ajaran }}</td>
                                <td class="text-center fw-bold">{{ $c->anggotaKelas->count() }} Siswa</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
