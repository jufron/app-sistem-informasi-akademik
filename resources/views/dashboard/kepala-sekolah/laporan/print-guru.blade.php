    ...
    <div class="container-fluid">
        ...
        <div class="row mb-4">
            <div class="col-12 text-center">
                <h5 class="fw-bold text-decoration-underline">LAPORAN DATA TENAGA PENDIDIK (GURU)</h5>
            </div>
        </div>
                    ...
                    <tbody>
                        @foreach($teachers as $idx => $t)
                            <tr>
                                ...
                                <td class="text-center font-mono">{{ $t->nip ?? '-' }}</td>
                                <td class="fw-bold">{{ $t->nama_lengkap }}</td>
                                <td>
                                    @php
                                        $mapels = $t->mataPelajaran->pluck('nama')->toArray();
                                    @endphp
                                    {{ count($mapels) > 0 ? implode(', ', $mapels) : '-' }}
                                </td>
