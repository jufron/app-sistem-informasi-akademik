<x-dashboard.layoutDashboard.app title="Ubah Jadwal Pelajaran">
    <x-slot:myStyle>
        <style>
            .color-option-label {
                width: 42px;
    ...
    <div class="card">
        <div class="card-header">
            <h4 class="card-title"><i class="fa fa-edit me-2"></i> Ubah Jadwal Pelajaran</h4>
        </div>

        <form action="{{ route('dashboard.jadwal-pelajaran.update', $jadwalPelajaran->id) }}" method="post">
            @csrf
            @method('PUT')
            <div class="card-body">
                    ...
                    <div class="col-md-3 mb-3">
                        <div class="form-group @error('ganjil_genap') has-error @enderror">
                            ...
                            <select class="form-select @error('ganjil_genap') is-invalid @enderror" name="ganjil_genap" required>
                                ...
                                <option value="Ganjil" {{ old('ganjil_genap', $jadwalPelajaran->ganjil_genap) == 'Ganjil' ? 'selected' : '' }}>Ganjil</option>
                                <option value="Genap" {{ old('ganjil_genap', $jadwalPelajaran->ganjil_genap) == 'Genap' ? 'selected' : '' }}>Genap</option>
                            </select>
                            @error('ganjil_genap')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="form-group @error('jam_mulai') has-error @enderror">
                            ...
                            <input type="time" name="jam_mulai" class="form-control @error('jam_mulai') is-invalid @enderror" value="{{ old('jam_mulai', date('H:i', strtotime($jadwalPelajaran->jam_mulai))) }}" required>
                            @error('jam_mulai')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="form-group @error('jam_selesai') has-error @enderror">
                            ...
                            <input type="time" name="jam_selesai" class="form-control @error('jam_selesai') is-invalid @enderror" value="{{ old('jam_selesai', date('H:i', strtotime($jadwalPelajaran->jam_selesai))) }}" required>
                            @error('jam_selesai')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="form-group @error('ruangan') has-error @enderror">
                            ...
                            <input type="text" name="ruangan" class="form-control @error('ruangan') is-invalid @enderror" value="{{ old('ruangan', $jadwalPelajaran->ruangan) }}" placeholder="Contoh: Ruang Kelas X-A, Laboratorium Komputer" max="100">
                            @error('ruangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        <div class="form-group">
                            ...
                            <div class="d-flex flex-wrap gap-3 align-items-center">
                                ...
                                <div class="color-option-wrapper">
                                    <label class="color-option-label active" style="background-color: #1572e8;" data-color="#1572e8" title="Cobalt Blue">
                                        <input type="radio" name="preset_warna" value="#1572e8" class="d-none">
                            ...
                            <input type="hidden" name="warna" id="final-color-input" value="{{ old('warna', $jadwalPelajaran->warna) }}">
                            @error('warna')
            ...
            <div class="card-action bg-light py-3 border-top d-flex justify-content-start">
                <button type="submit" class="btn btn-success btn-round"><i class="fa fa-save me-1"></i> Perbarui Jadwal</button>
    ...
    <x-slot:myScript>
        <script>
            jQuery(function($) {
                ...
                // Sync the active color state from the initial/old database color
                var initialColor = $('#final-color-input').val();
                if (initialColor) {
                    var presetMatched = false;
                    $('.color-option-label').each(function() {
                        if ($(this).data('color').toLowerCase() === initialColor.toLowerCase()) {
                            $('.color-option-label').removeClass('active');
                            $('.color-option-label i').addClass('d-none');
                            $(this).addClass('active');
                            $(this).find('i').removeClass('d-none');
                    ...
                    if (!presetMatched) {
                        ...
                        $('#preset-custom').prop('checked', true);
                        $('#custom-color-container').addClass('border-dark');
                        $('#custom-color-picker').val(initialColor);
                    }
                }
            });
        </script>
